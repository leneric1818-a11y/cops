#!/bin/bash
#SBATCH --job-name=qwen3_layersep
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --partition=p6
...
DATA_PATH="${DATA_PATH:-outputs/metrics/persona_pairs_openness_gpt54mini_1000_flat.jsonl}"
...
OUTPUT_DIR="${OUTPUT_DIR:-outputs/metrics/layerwise_separability_openness_gpt54mini_1000}"
STYLE_NEG="${STYLE_NEG:-}"
STYLE_POS="${STYLE_POS:-}"
if [[ -n "$STYLE_NEG" && -n "$STYLE_POS" ]]; then
  STYLES="${STYLES:-$STYLE_NEG,$STYLE_POS}"
else
  STYLES="${STYLES:-defensive,open}"
fi
