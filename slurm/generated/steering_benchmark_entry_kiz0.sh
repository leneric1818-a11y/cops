#!/bin/bash
#SBATCH --job-name=steering_benchmark
#SBATCH --nodes=1
#SBATCH --ntasks=1
#SBATCH --partition=p6
...
echo "=================================================================="
echo "Qwen Steering Benchmark Entry at $(date)"
...
if command -v conda >/dev/null 2>&1; then
  ...
  echo "WARNING: conda command not available; falling back to sourcing the env directly."
  # Fallback keeps the job usable if the Anaconda module is unavailable on a node.
  # The preferred path is conda activation above.
  source "$CONDA_ENV_DIR/bin/activate"
fi

# Some cluster nodes keep the module Python ahead of the activated env in PATH.
# Force the prepared env bin directory to the front so all downstream checks and
# subprocesses resolve the intended interpreter.
...
if [ -z "${RUN_SPEC_PATH:-}" ]; then
  echo "ERROR: RUN_SPEC_PATH must be set."
  exit 1
fi
